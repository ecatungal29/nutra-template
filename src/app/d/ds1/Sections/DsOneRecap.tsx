import React from "react";

export default function DsOneRecap() {
    return (
        <>
            <div className="w-full md:w-9/12 mx-auto mt-10 ">
                <div className="w-full mx-auto md:w-11/12">
                    <h1 className="text-2xl text-center md:text-[28px] font-bold mb-5 mx-6">
                        Because as you know by now...
                    </h1>

                    <h3 className=" px-5 text-base mb-3">
                        The Maximum Benefits and Greatest Lasting Results
                        Typically Come To Those Who Take ProstaFlow For A
                        Minimum Of 90 days...
                    </h3>
                </div>

                <div className="w-11/12 md:w-11/12 mx-auto mt-10 bg-[#1e103e] border rounded-3xl p-5 shadow md:p-10 max-w-screen-xl">
                    <div className="w-full md:w-12/12 mx-auto  mb-5">
                        <h1 className="p-5  md:p-0 md:mb-10 text-center text-xl text-[28px] text-white font-bold">
                            Here&apos;s a short recap about the benefits that
                            taking ProstaFlow long-term will bring you:
                        </h1>
                    </div>

                    <div className="flex justify-center p-8 md:p-0">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div className="mt-5 p-3 border border-white flex flex-col items-center bg-gradient-to-t from-[#26144d] to-[#26144d] rounded-xl">
                                <div
                                    className="-mt-9 bg-[#26144d] border border-white text-white font-bold rounded-full"
                                    style={{
                                        width: "50px",
                                        height: "50px",
                                        display: "flex",
                                        justifyContent: "center",
                                        alignItems: "center",
                                    }}
                                >
                                    1
                                </div>
                                <p className="text-center text-base mt-2 mb-4 text-white">
                                    Maintaining Healthy Blood Sugar Levels *
                                </p>
                            </div>

                            <div className="mt-5 p-3 border border-white flex flex-col items-center bg-gradient-to-t from-[#26144d] to-[#26144d] rounded-xl">
                                <div
                                    className="-mt-9 bg-[#26144d] border border-white text-white font-bold rounded-full"
                                    style={{
                                        width: "50px",
                                        height: "50px",
                                        display: "flex",
                                        justifyContent: "center",
                                        alignItems: "center",
                                    }}
                                >
                                    2
                                </div>
                                <p className="text-center text-base mt-2 mb-4 text-white">
                                    Supporting Balanced Blood Sugar Naturally *
                                </p>
                            </div>

                            <div className="mt-5 p-3 border border-white flex flex-col items-center bg-gradient-to-t from-[#26144d] to-[#26144d] rounded-xl">
                                <div
                                    className="-mt-9 bg-[#26144d] border border-white text-white font-bold rounded-full"
                                    style={{
                                        width: "50px",
                                        height: "50px",
                                        display: "flex",
                                        justifyContent: "center",
                                        alignItems: "center",
                                    }}
                                >
                                    3
                                </div>
                                <p className="text-center text-base mt-2 mb-4 text-white">
                                    Aiding Energy Production *
                                </p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </>
    );
}
